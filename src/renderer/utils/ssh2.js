...
const ssh2 = {
    ...
    //执行多行命令
    async SSH2ExecCommands(props) {
        return new Promise((resolve, reject) => {
            Connect(props.nodeinfo).then(async ({
                ssh
            }) => {
                // 异步操作数组
                const asyncOperations = [
                    () => new Promise((resolve) => {
                        ssh.exec("sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart", [], {})
                            .then((res) => {
                                console.log("res1 sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart")
                                resolve()
                            })
                            .catch((err) => {
                                reject(err)
                    ...
                    () => new Promise((resolve) => {
                        ssh.exec("sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart", [], {})
                            .then((res) => {
                                console.log("res2 sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart")
                                resolve()
                            })
                            .catch((err) => {
                                reject(err)
                    ...
                    () => new Promise((resolve) => {
                        ssh.exec("sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart", [], {})
                            .then((res) => {
                                console.log("res3 sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart")
                                resolve()
                            })
                            .catch((err) => {
                                reject(err)
                    ...
                    () => new Promise((resolve) => {
                        ssh.exec("sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart", [], {})
                            .then((res) => {
                                console.log("res4 sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart")
                                resolve()
                            })
                            .catch((err) => {
                                reject(err)
                ...
                // 按顺序执行异步操作
                const sequentialPromises = asyncOperations.reduce(
                    (promiseChain, currentOperation) => promiseChain.then(currentOperation),
                    Promise.resolve() // 初始的空解决的promise
                );

                console.log(new Date())
                sequentialPromises.then(() => {
                    console.log("ok")
                    console.log(new Date())
                    resolve()
                }).catch(console.error);
                ...
                // const arr = [
                //     'sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart',
                //     'sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart'
                // ]
                // const arrPromise = []
                ...
                // }
                // const doTask = arrPromise.reduce((prev, next) => prev.then(() => next()), Promise.resolve());
    ...
    SSH2ExecCommand(props) {
        return new Promise((resolve, reject) => {
            Connect(props.nodeinfo).then(({
                ...
                ssh.exec('sh /home/kone/kone_kswap/kone-kswap/bin/service.sh restart', [], {}).then((res) => {
                    console.log("res", res)
                    resolve()
                })
